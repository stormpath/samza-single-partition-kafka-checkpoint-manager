...
import com.stormpath.samza.lang.*;
...
public class ZookeeperCheckpointManager implements CheckpointManager {
    ...
    public ZookeeperCheckpointManager(Supplier<CuratorFramework> curatorFactory, String jobPath) {
        Assert.hasText(jobPath, "jobPath cannot be null or empty.");
        this.tasksPath = jobPath + "/tasks";
        this.curatorFactory = Assert.notNull(curatorFactory, "Curator Supplier cannot be null.");
    }
    ...
    private void doStart() throws Exception {
        ...
        this.curator = curatorFactory.get();
        this.curator.start();

        //ensure base path node exists:
        try {
            String result = curator.create().creatingParentsIfNeeded().forPath(tasksPath);
        ...
        for (TaskName tn : registeredTaskNames) {
            ...
            try {
                String result = curator.create().creatingParentsIfNeeded().forPath(checkpointPath);
